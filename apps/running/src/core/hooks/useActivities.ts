...
const activitiesUrl = `${import.meta.env.BASE_URL}data/activities.json`;
...
const loadActivityData = () => {
  activityDataPromise ??= fetch(activitiesUrl, { cache: 'no-cache' })
    .then((response) => {
      if (!response.ok)
    ...
    .then((data) => {
      if (!Array.isArray(data)) throw new Error('Invalid activities response');
