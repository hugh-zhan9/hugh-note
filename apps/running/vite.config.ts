...
import { activityDataPlugin } from './plugins/activity-data';
...
export default defineConfig({
  ...
  plugins: [
      ...
      transformIndexHtml: {
        ...
        handler() {
          return [
            {
              tag: 'script',
              children: ['themetoggle.js', 'running-route.js']
                .map((name) =>
                  readFileSync(
    ...
    activityDataPlugin(path.resolve(__dirname, 'src/static/activities.json')),
    react(),
    tailwindcss(),
    yaml(),
    svgr({
