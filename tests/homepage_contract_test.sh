...
assert_contains "$ROOT_DIR/layouts/index.html" 'homepage-identity-meta'
assert_contains "$ROOT_DIR/layouts/index.html" 'homepage-running-metrics'
...
assert_contains "$ROOT_DIR/layouts/index.html" 'range $pageRank, $page := $crazyTalkPages'
assert_contains "$ROOT_DIR/layouts/index.html" 'data-source-rank'
...
assert_contains "$ROOT_DIR/static/js/homepage.js" 'CRAZY_TALK_INTERVAL_MS = 8000'
assert_contains "$ROOT_DIR/static/js/homepage.js" 'CRAZY_TALK_FILE_LIMIT = 30'
assert_contains "$ROOT_DIR/static/js/homepage.js" '1 / Math.sqrt(file.sourceRank + 1)'
assert_contains "$ROOT_DIR/static/js/homepage.js" '.slice(0, CRAZY_TALK_FILE_LIMIT)'
assert_contains "$ROOT_DIR/static/js/homepage.js" 'Math.floor(Math.random() * (index + 1))'
assert_contains "$ROOT_DIR/static/js/homepage.js" 'homepage-running-percent'
assert_contains "$ROOT_DIR/static/js/homepage.js" '这个月暂时还没有跑步记录。'
...
assert_contains "$ROOT_DIR/assets/css/homepage.css" '.homepage-identity-meta'
assert_contains "$ROOT_DIR/assets/css/homepage.css" '.homepage-running-metrics'
assert_contains "$ROOT_DIR/assets/css/homepage.css" 'border-left: 1px solid var(--homepage-border)'
...
assert_contains "$ROOT_DIR/assets/css/homepage.css" 'border-bottom: 1px solid var(--homepage-border)'
